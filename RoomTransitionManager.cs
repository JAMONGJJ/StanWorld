...
namespace stanworld
{
    ...
    public class RoomTransitionManager
    {
        ...
        public void StartRoomTransition()
        {
            try
            {
                ...
                LoadingInstance.Instance.StartLoadingTransition(delegate
                {
                    ...
                    else    // 로비 최초 진입(LoginState, CreateCharacterState) -> No need to Leave room, Just Join room.
                    {
                        FirstJoinStanWorld();
        ...
        {
            GameInstance.Instance.Send_PACKET_CHECKIN_NOTI();   // 서버에서 경우에 따라 응답이 안 올 수 있음. 따라서 requestManager에서 관리 안 함.
        ...
        /// <summary>
        /// 파티룸 조인할 때, 서버로부터 필요한 패킷을 모두 받은 후 그 정보들로 상태를 초기화(스트리밍 스크린 등등)
        /// </summary>
